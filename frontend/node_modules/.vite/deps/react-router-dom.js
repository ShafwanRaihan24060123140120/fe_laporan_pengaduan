import { n as __commonJS, r as __toESM, t as require_react } from "./react-D1ZggjGU.js";
import { t as require_react_dom } from "./react-dom-VxjU054X.js";

//#region ../node_modules/react-router/dist/development/chunk-EPOLDU6W.mjs
...
function createRouter(init) {
	...
	async function startNavigation(historyAction, location$1, opts) {
		...
		let { shortCircuited, matches: updatedMatches, loaderData, errors } = await handleLoaders(request, location$1, matches, scopedContext, fogOfWar.active, loadingNavigation, opts && opts.submission, opts && opts.fetcherSubmission, opts && opts.replace, opts && opts.initialHydration === true, flushSync, pendingActionResult, opts && opts.callSiteDefaultShouldRevalidate);
	...
	async function handleLoaders(request, location$1, matches, scopedContext, isFogOfWar, overrideNavigation, submission, fetcherSubmission, replace2, initialHydration, flushSync, pendingActionResult, callSiteDefaultShouldRevalidate) {
	...
	async function startRedirectNavigation(request, redirect2, isNavigation, { submission, fetcherSubmission, preventScrollReset, replace: replace2 } = {}) {
		...
		location$1 = normalizeRedirectLocation(location$1, new URL(request.url), basename, init.history);
...
function normalizeRedirectLocation(location$1, currentUrl, basename, historyInstance) {
	...
	try {
		let url = historyInstance.createURL(location$1);
		if (invalidProtocols.includes(url.protocol)) throw new Error("Invalid redirect location");
	} catch (e) {}
	return location$1;
}
...
function useNavigateStable() {
	...
	return import_react.useCallback(async (to, options = {}) => {
		warning(activeRef.current, navigateEffectWarning);
		if (!activeRef.current) return;
		if (typeof to === "number") await router$1.navigate(to);
...
function RouterProvider({ router: router$1, flushSync: reactDomFlushSyncImpl, onError, unstable_useTransitions }) {
	unstable_useTransitions = useIsRSCRouterContext() || unstable_useTransitions;
	let [_state, setStateImpl] = import_react.useState(router$1.state);
...
function getSingleFetchDataStrategyImpl(getRouter, getRouteInfo, fetchAndDecode, ssr, basename, trailingSlashAware, shouldAllowOptOut = () => true) {
	return async (args) => {
		...
		return singleFetchLoaderNavigationStrategy(args, router$1, getRouteInfo, fetchAndDecode, ssr, basename, trailingSlashAware, shouldAllowOptOut);
	};
}
...
async function singleFetchLoaderNavigationStrategy(args, router$1, getRouteInfo, fetchAndDecode, ssr, basename, trailingSlashAware, shouldAllowOptOut = () => true) {
...
var Link = import_react.forwardRef(function LinkWithRef({ onClick, discover = "render", prefetch = "none", relative, reloadDocument, replace: replace2, state, target, to, preventScrollReset, viewTransition, unstable_defaultShouldRevalidate, ...rest }, forwardedRef) {
	let { basename, unstable_useTransitions } = import_react.useContext(NavigationContext);
	...
	let link = /* @__PURE__ */ import_react.createElement("a", {
		...rest,
		...prefetchHandlers,
		href: parsed.absoluteURL || href$1,
...
function useLinkClickHandler(to, { target, replace: replaceProp, state, preventScrollReset, relative, viewTransition, unstable_defaultShouldRevalidate, unstable_useTransitions } = {}) {
	let navigate = useNavigate();
	let location$1 = useLocation();
	...
	return import_react.useCallback((event) => {
		if (shouldProcessLinkClick(event, target)) {
			event.preventDefault();
			let replace2 = replaceProp !== void 0 ? replaceProp : createPath(location$1) === createPath(path);
...
function useSubmit() {
	...
	let routerFetch = router$1.fetch;
	let routerNavigate = router$1.navigate;
...
function useFetcher({ key } = {}) {
	...
	let { deleteFetcher, getFetcher, resetFetcher, fetch: routerFetch } = router$1;
	import_react.useEffect(() => {
	...
	let load = import_react.useCallback(async (href$1, opts) => {
		...
		await routerFetch(fetcherKey, routeId, href$1, opts);
	}, [
		fetcherKey,
		routeId,
		routerFetch
...
var require_set_cookie = /* @__PURE__ */ __commonJS({ "../node_modules/set-cookie-parser/lib/set-cookie.js": ((exports, module) => {
...
//#endregion
//#region ../node_modules/react-router/dist/development/chunk-FNSCYPCZ.mjs
/**
* react-router v7.12.0
*
* Copyright (c) Remix Software Inc.
...
async function handleDocumentRequest(serverMode, build, staticHandler, request, loadContext, handleError, isSpaMode, criticalCss) {
	try {
		if (request.method === "POST") throwIfPotentialCSRFAttack(request.headers, Array.isArray(build.allowedActionOrigins) ? build.allowedActionOrigins : []);
...
async function routeRSCServerRequest({ request, serverResponse, createFromReadableStream, renderHTML, hydrate: hydrate$1 = true }) {
	...
	try {
		...
		if (!hydrate$1) return new Response(html.pipeThrough(redirectTransform), {
			status,
			statusText,
			headers
		});
		...
		try {
			...
			if (!hydrate$1) return new Response(html.pipeThrough(retryRedirectTransform), {
				status,
				statusText,
				headers
			});
